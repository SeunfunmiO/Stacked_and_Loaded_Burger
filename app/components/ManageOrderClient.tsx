...
//             order.email.includes(searchQuery);
...
//                             {(['all', 'pending', 'confirmed', 'ready', 'out-for-delivery', 'delivered', 'cancelled'] as FilterStatus[]).map((status) => (
//                                 <button
//                                     key={status}
//                                     onClick={() => setFilterStatus(status)}
//                                     className={`px-4 py-2 rounded-lg font-medium transition-colors ${filterStatus === status
//                                         ? 'bg-linear-to-r from-sandbrown to-[#f4a261] text-white'
